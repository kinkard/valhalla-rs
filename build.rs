...
fn main() {
    ...
    let mut cmake_config = cmake::Config::new("valhalla");
    
    // Force vcpkg to use "Classic Mode" on Windows
    if cfg!(target_os = "windows") {
        cmake_config.define("VCPKG_MANIFEST_MODE", "OFF");
    ...
    let mut bridge = cxx_build::bridges(["src/actor.rs", "src/config.rs", "src/lib.rs"]);
    bridge
    ...
    // Windows-specific cxx bridge configuration
    if cfg!(target_os = "windows") {
        let vcpkg_base = std::env::var("VCPKG_ROOT").expect("VCPKG_ROOT environment variable must be set");
        let vcpkg_include = format!(r"{}\installed\x64-windows\include", vcpkg_base);
        
        bridge
            .define("ABSL_CONSUME_DLL", None)
    ...
    if cfg!(target_os = "windows") {
        let vcpkg_base = std::env::var("VCPKG_ROOT").expect("VCPKG_ROOT environment variable must be set");
        let vcpkg_root = format!(r"{}\installed\x64-windows", vcpkg_base);
        // 1. Valhalla Search Path
        println!("cargo:rustc-link-search=native={dst_path}/build/src");

        // 2. VCPKG Search Path & Suffix Logic
        let (vcpkg_lib_path, suffix) = if build_type == "Debug" {
            (format!("{}\\debug\\lib", vcpkg_root), "d")
        } else {
            (format!("{}\\lib", vcpkg_root), "")
        };

        println!("cargo:rustc-link-search=native={}", vcpkg_lib_path);

        // 3. Core Libraries
        println!("cargo:rustc-link-lib=static=valhalla");
        println!("cargo:rustc-link-lib=static=zlib{}", suffix); 
        println!("cargo:rustc-link-lib=static=libprotobuf-lite{}", suffix);

        // 4. Dynamic Abseil/Protobuf Helper Logic
        // This loop looks at your folder and links everything starting with absl, abseil, or utf8
        if let Ok(entries) = fs::read_dir(&vcpkg_lib_path) {
            for entry in entries.flatten() {
                let name = entry.file_name().to_string_lossy().to_string();
                if (name.starts_with("absl") || name.starts_with("abseil") || name.starts_with("utf8")) 
                    && name.ends_with(".lib") 
                {
                    let lib_name = name.trim_end_matches(".lib");
                    println!("cargo:rustc-link-lib=static={}", lib_name);
                }
            }
        }

        // 5. Windows System Libraries (Required for OS features)
        let sys_libs = ["shell32", "ole32", "advapi32", "ws2_32", "user32", "userenv", "dbghelp", "bcrypt"];
        for lib in sys_libs {
