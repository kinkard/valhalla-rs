/// Fixed-size bitset over the range [0, size), which is set at creation and cannot be extended.
/// Suits `GraphId::id()`, a dense index into its own tile's node array.
...
impl BitSet {
    ...
    /// Adds an index, returning true if it wasn't already present. Out-of-bounds indices are
...
mod tests {
    ...
    fn rounds_up_to_whole_words() {
        let mut bitset = BitSet::new(126);
        // Within the allocated words, so capacity behaves as 128.
