//! Snaps a coordinate onto the graph with `Actor::locate`, picking the edges the router would.
//! `/locate` has no protobuf serializer and always answers JSON, hence the serde structs below.
...
pub fn locate(actor: &mut Actor, coord: LatLon, search_radius: u32) -> Result<Vec<GraphId>> {
    let request = proto::Options {
        ...
        // Load-bearing: `edge_id` is only written in the serializer's verbose branch.
